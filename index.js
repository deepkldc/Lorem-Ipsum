const text = [
  ...
  const form = document.querySelector(".genrator");
  const amount=document.querySelector(".no-para");
  ...
  form.addEventListener("submit",function(e){
   ...
   const value=parseInt(amount.value);
    ...
    {const random=Math.floor(Math.random()*text.length);
        result.innerHTML=`<p class="para">${text[random]}</p>`;
    }
    else
    {
     let temptext =text.slice(0,value);
      //result.innerHTML =`<p class="para">${temptext}</p>`;
    //console.log(temptext);
    
     let temp =temptext.map(function(item){
